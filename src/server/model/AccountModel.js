...
const AccountModel = {
    ...
    forget: async (reqData) => {
        const sql = `UPDATE fen_account SET a_password='${reqData.a_password}' WHERE a_id=${reqData.a_id}`
        const data = await _connect(sql)
        return data['affectedRows']
    },
    utils: {
        ...
        getDataByPwd: async (reqData) => {
            const sql = `SELECT * FROM fen_account WHERE a_id='${reqData.a_id}' AND a_password='${reqData.a_password}'`
            const data = await _connect(sql)
            return data[0]
        },
        getDataByEmail: async (reqData) => {
            const sql = `SELECT * FROM fen_account WHERE a_id='${reqData.a_id}' AND a_email='${reqData.a_email}'`
            const data = await _connect(sql)
            return data[0]
        },
        ...
        setPwdTemp: async (reqData) => {
            const sql = `UPDATE fen_account SET a_pwd_temp='${reqData.a_password}||${reqData.currentTime}||${reqData.exp}' WHERE a_id=${reqData.a_id}`
            const data = await _connect(sql)
            return data['affectedRows']
        },
        getPwdTemp: async (reqData) => {
            const sql = `SELECT a_pwd_temp FROM fen_account WHERE a_id='${reqData.a_id}'`
            const data = await _connect(sql)
            return data[0]
        }
    }
